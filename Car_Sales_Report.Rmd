...
Due to the nature of our dataset and the some of the linear relations between variables we observed we'll start by trying linear models and more complex models later.
...
Divergence from the diagonal line highlights prediction errors, with each model’s performance visibly differentiated by color.

```{r Residuals Plot, echo=FALSE}
combined_predictions <- data.frame(
  Actual = exp(c(testData$price, testData$price, testData$price)),
  Predicted = exp(c(predictions_lr, predictions_elasticnet, predictions_lightgbm)),
  Model = rep(c("Baseline Linear Regression", "Elastic Net", "LightGBM"), each = nrow(testData))
)

ggplot(combined_predictions, aes(x = Actual, y = Predicted, color = Model)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, color = "black", linetype = "dashed") +
  labs(x = "Actual Values", y = "Predicted Values", title = "Actual vs Predicted Values by Model") +
  theme_minimal()

ggsave("combined_predictions_plot.png", plot = p, width = 10, height = 10)
knitr::include_graphics("combined_predictions_plot.png")

```

As we see, the LightGBM model has the best performance, with predictions closely aligned with the actual values.
...
```{r Prediction Error Plot, echo=FALSE}
ggplot(combined_predictions, aes(x = Actual, y = Predicted - Actual, color = Model)) +
  geom_point() +
  geom_hline(yintercept = 0, color = "black", linetype = "dashed") +
  labs(x = "Actual Values", y = "Residuals", title = "Residuals vs Actual Values by Model")
...
The learning curve displays the model’s performance improvement over training iterations, helping to evaluate learning efficiency and convergence behavior.
If the curve plateaus, additional training data or iterations may not significantly improve model performance, indicating potential model limits.

```{r , echo=FALSE}
lgb.plot.learning(lightgbm_model)
```
...
We could collect more data to make our dataset more representative of all used cars.
We could try different models that do not assume a linear relationship between the features and the car price, such as decision tree or neural network models.
We could also try to include more features in our models, such as the car's condition or the seller's negotiation skills.
Finally, we could validate our models on a separate test set to ensure that they generalize well to new data.
