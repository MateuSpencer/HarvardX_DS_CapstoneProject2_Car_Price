...
```{r bar plots, echo=FALSE, fig.height=7, fig.width=6}
all_cars %>%
...
```
Lets vizualize the distibutions of the numerical variables.
```{r boxplots numerical, echo=FALSE, fig.height=10, fig.width=10}
...
```
#### Year
There is at least one clear error in the year since one car is from the future, so we will remove entries whose year is greater than 2024 and also the outliers with years below 1990.
...
```{r tax high, echo=FALSE, fig.height=10, fig.width=10}
...
```{r tax low, echo=FALSE, fig.height=10, fig.width=10}
...
```{r engineSize big, echo=FALSE, fig.height=10, fig.width=10}
...
```{r after outlier, echo=FALSE, fig.height=10, fig.width=10}
...
First we'll start by encoding the categorical variables using one-hot encoding.
...
As mentioned before we will perform a log transformation to normalize the values of mileage and price to improve performance in models that assume normality.
...
We will be building several models to predict the price of the cars.
We will evaluate the performance of each model using appropriate metrics and select the best model based on these metrics.

# 3. Model Building
We will use models from the caret package with cross validation to evaluate the performance of the models and select the best one based on the RMSE and R2 values.
...
predictions <- predict(lr_model, newdata = testData)

evaluate_model(predictions, "Baseline Linear Regression", results)

rm(lr_model, train_control, predictions)
```
...
predictions <- predict(elasticnet_model, newdata = testData)

evaluate_model(predictions, "Elastic Net", results)

rm(elasticnet_model, train_control, tuneGrid, predictions)
```
...
predictions <- predict(lightgbm_model, newdata = as.matrix(testData_X))

results <- evaluate_model(predictions, "LightGBM", results)
